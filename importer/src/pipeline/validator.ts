...
const metadataRefSchema = z.object({
  // MAL/AniList id — AniList genres carry only a name, so id is optional.
  malId: z.number().int().positive().nullable().optional(),
  name: z.string().min(1),
});

export const animeRowSchema = z
  .object({
    ...
    coverImageColor: z.string().nullable().optional(),
    trailerId: z.string().nullable().optional(),
    trailerSite: z.string().nullable().optional(),
    trailerThumbnail: z.string().nullable().optional(),
    nextAiringAt: z.string().nullable().optional(),
  })
  .refine((r) => r.idMal != null || r.anilistId != null, {
    message: "row must carry at least one external id (idMal or anilistId)",
  });
...
export const validateAnimeRow: Validator<NormalizedAnime> = (row) => {
  ...
  if (parsed.success) return { ok: true, row: parsed.data as NormalizedAnime };
